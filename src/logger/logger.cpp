...
void Logger::init(const std::string& logger_name,
    ...
    try {
        ...
        // Add console output sink (use stderr to avoid polluting stdout in stdio/MCP mode)
        if (console_output) {
...
std::shared_ptr<spdlog::logger> Logger::getLogger() {
    if (!m_initialized) {
        ...
        init("mcp_server", "../../logs/server.log", 10 * 1024 * 1024, 5, true);
    }
    return m_logger;
}
...
void Logger::shutdown() {
    ...
    try {
        ...
        // 关闭 spdlog 注册表并清理资源。
        // 注意：在进程退出阶段，静态对象析构顺序不可控，spdlog 内部状态可能已销毁。
        // 这里必须吞掉异常，避免测试进程在退出时被 terminate。
        spdlog::shutdown();
